...
- The parameters bar is quieter on a wide screen. Every target chip repeated the comparison that the menu beside it already names, so five targets meant five copies of the same symbol; the chips now show just the number. Their remove control appears when you point at a chip or tab to it rather than sitting on every one, and the box for adding a target no longer turns into a dash that looked like a broken chip when the list is full. With five targets and five pool counts the bar is about 300px shorter.
