...
- Head-to-head shows the first twelve rolls. It had no limit at all, so a hundred rolls meant a hundred columns and ten thousand cells, each one its own comparison and its own tab stop. Twelve is what the picture of that view already used, so the two now always agree, and the note points at Roll-off, which does rank every roll at once.
...
- A big table gets a comparison chart again. Past twenty rolls the chart used to go blank and say it was disabled, which is easy to land on in one paste of a shared link. It now draws the first twenty and says "Showing the first 20 of 21 rolls" under the legend, the way every other card in the app already states what it left out. A table mixing totals and successes splits that budget between its two charts in proportion, so neither kind is pushed off. Nothing changes for a table of twenty or fewer: it drew everything before and still does.
