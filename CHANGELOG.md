...
- The table now appears only when it fits. It used to arrive at 768px while needing about 1009px, so from there up to roughly 1050px it scrolled sideways and the row buttons, delete included, sat off the right edge with nothing to suggest they were there. The cards cover those widths now. From 992px the table returns without Range and the shape sparkline, the two columns the chart underneath already shows, which is what lets everything else stay on screen; both come back at 1280px.
